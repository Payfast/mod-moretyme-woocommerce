...
const  DISPLAY_STYLE_NONE = 'style="display: none;"';
    ...
    <script type="text/javascript">
      jQuery(function ($) {
        ...
        $('.pf-mt-input').change(function () {
          ...
          s.src = 'https://content.payfast.io/widgets/moretyme/widget.min.js'
        })
      })
    </script>
    <section>
        ...
        if (get_option('woocommerce_currency') == 'ZAR') {
            ...
            <p>Create a personalised widget for your product web pages and increase sales on your site. The widget
                informs your customers how much they need to pay upfront and for the next two payments with
                MoreTyme.</p>

            <div id="pf-mt-preview" style="background-color: <?php
            echo $settings['theme'] == 'dark' ? '#000000' : '#ffffff' ?>;">
                <script async src="https://content.payfast.io/widgets/moretyme/widget.min.js?<?php
                echo str_replace('%23', '#', http_build_query($settings)) ?>" type="text/javascript"></script>
            </div>
            ...
            <div class="tab-col mode">
                ...
                <input type="radio" name="theme" class="pf-mt-input" value="light" id="light-mode"
                    <?php
                    echo $settings['theme'] != 'dark' ? 'checked' : '' ?>/>
                ...
                <input type="radio" name="size" class="pf-mt-input" value="standard" id="size-standard"
                    <?php
                    echo $settings['size'] != 'small' ? 'checked' : '' ?>/>
            ...
            <div class="tab-col fonts">
                ...
                <select name="font" class="pf-mt-input">
                    ...
                    echo $settings['font'] == 'Arial' ? 'selected' : '' ?>>Arial
                    </option>
                    <option value="Garamond" <?php
                    echo $settings['font'] == 'Garamond' ? 'selected' : '' ?>> Garamond
                    </option>
                    <option value="Helvetica" <?php
                    echo $settings['font'] == 'Helvetica' ? 'selected' : '' ?>> Helvetica
                    </option>
                    <option value="Lato" <?php
                    echo $settings['font'] == 'Lato' ? 'selected' : '' ?>> Lato
                    </option>
                    <option value="Merriweather" <?php
                    echo $settings['font'] == 'Merriweather' ? 'selected' : '' ?>> Merriweather
                    </option>
                    <option value="Merriweather-Sans" <?php
                    echo $settings['font'] == 'Merriweather-Sans' ? 'selected' : '' ?>> Merriweather Sans
                    </option>
                    <option value="Montserrat" <?php
                    echo $settings['font'] == 'Montserrat' ? 'selected' : '' ?>> Montserrat
                    </option>
                    <option value="Open-Sans" <?php
                    echo $settings['font'] == 'Open-Sans' ? 'selected' : '' ?>> Open Sans
                    </option>
                    <option value="Oswald" <?php
                    echo $settings['font'] == 'Oswald' ? 'selected' : '' ?>> Oswald
                    </option>
                    <option value="PT-Sans" <?php
                    echo $settings['font'] == 'PT-Sans' ? 'selected' : '' ?>> PT Sans
                    </option>
                    <option value="Raleway" <?php
                    echo $settings['font'] == 'Raleway' ? 'selected' : '' ?>> Raleway
                    </option>
                    <option value="Roboto" <?php
                    echo $settings['font'] == 'Roboto' ? 'selected' : '' ?>> Roboto
                    </option>
                    <option value="Source-Sans-Pro" <?php
                    echo $settings['font'] == 'Source-Sans-Pro' ? 'selected' : '' ?>> Source Sans Pro
                    </option>
                    <option value="Times-New-Roman" <?php
                    echo $settings['font'] == 'Times-New-Roman' ? 'selected' : '' ?>> Times-New-Roman
                    </option>
                    <option value="Verdana" <?php
                    echo $settings['font'] == 'Verdana' ? 'selected' : '' ?>> Verdana
                    </option>
                </select>

                <div id="payfast-font-color" <?php
                echo $settings['theme'] == 'dark' ? DISPLAY_STYLE_NONE : '' ?>>
                    <h4>Select Font Color</h4>
                    <input type="color" name="font-color" class="pf-mt-input" value="<?php
                    echo $settings['font-color'] ?: '#041B2B' ?>"/>
                </div>
                <div id="payfast-link-color" <?php
                echo $settings['size'] == 'small' ? DISPLAY_STYLE_NONE : '' ?>>
                    <h4>'Learn More' link colour</h4>
                    <input type="color" name="link-color" class="pf-mt-input" value="<?php
                    echo $settings['link-color'] ?: '#177EE8' ?>">
                </div>
            </div>
            <?php
        } else {
            ...
            <h4>To change your shop currency settings go to: WooCommerce > Settings > General > Currency Options , and
                select ZAR from the Currency drop down.</h4>
            <?php
        }
        ?>
    </section>
